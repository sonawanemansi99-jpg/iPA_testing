...
class _RegisterSuperAdminPageState extends State<RegisterSuperAdminPage> {
  ...
  static const rootPurple = Color(0xFF512DA8); // Distinct Royal Color for Super Admin
        ...
        child: Wrap(
          children: [
            Padding(
              ...
              child: const Text("Profile Photo (Optional)", style: TextStyle(color: gold, fontWeight: FontWeight.bold, fontSize: 16)),
            ),
            ListTile(
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (_selectedImage != null) {
        uploadedPhotoUrl = await _mediaServices.uploadMediaToCloudinary(_selectedImage!);
                                ...
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      border: Border.all(color: _selectedImage != null ? gold : Colors.white30, width: 2),
                                      image: _selectedImage != null
                                          ? DecorationImage(image: FileImage(_selectedImage!), fit: BoxFit.cover)
                                          : null,
                                      boxShadow: [
                                        if (_selectedImage != null)
                                          BoxShadow(color: gold.withOpacity(0.4), blurRadius: 15, spreadRadius: 2)
                                      ],
                                    ),
                            ...
                            children: [
                              Expanded(
                                child: _buildInputField(
                                  ...
                                  maxLength: 10,
                                  validator: (val) => !RegExp(r"^[0-9]{10}$").hasMatch(val!) ? "Must be 10 digits" : null,
                                ),
                              ),
                              ...
                              Expanded(
                                child: _buildInputField(
                                  ...
                                  maxLength: 12,
                                  validator: (val) => !RegExp(r"^[0-9]{12}$").hasMatch(val!) ? "Must be 12 digits" : null,
                                ),
                              ),
                            ],
                          ),
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(shape: BoxShape.circle, color: rootPurple.withOpacity(0.3)),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("WARNING", style: TextStyle(color: Colors.redAccent, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 2)),
                const SizedBox(height: 4),
                const Text("Creating a Super Admin grants complete system control.", style: TextStyle(color: Colors.white70, fontSize: 12, height: 1.4)),
              ],
            ),
          )
        ],
  ...
  Widget _buildPasswordField() {
    return TextFormField(
      ...
      validator: (val) {
        ...
        if (!RegExp(r"^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=!]).*$").hasMatch(val)) {
