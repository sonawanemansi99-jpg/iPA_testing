...
class _CorporatorQRDownloadPageState extends State<CorporatorQRDownloadPage>
  ...
  // ── SPRING BOOT INTEGRATION LOGIC ──
  Future<void> fetchAndGenerateQR() async {
    try {
      // 1. Fetch the profile using our existing secure service
      final profileData = await _adminService.getCorporatorProfile();

      // 2. Extract the Admin Group ID and Name
      // (Ensure your Spring Boot AdminResponse includes adminGroupId and adminGroupName!)
      final fetchedId = profileData['adminGroupId']?.toString();
      final fetchedName =
          profileData['adminGroupName']?.toString() ?? 'Official Group';

      if (fetchedId == null || fetchedId.isEmpty) {
        throw Exception("Admin Group ID not assigned to this Corporator.");
      }

      if (mounted) {
        setState(() {
          adminGroupId = fetchedId;
          adminGroupName = fetchedName;
          // 3. Generate robust URL using ID, not name!
          url =
              "https://velvety-pithivier-1d0359.netlify.app/?adminGroupId=$adminGroupId";
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> downloadQR() async {
    ...
    try {
      ...
      final directory = await getApplicationDocumentsDirectory();
      // Saved cleanly with the Group ID
      final file = File("${directory.path}/group_${adminGroupId}_qr.png");
      await file.writeAsBytes(pngBytes);
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            // ── UPDATED: Admin Group Name label ──
                            Container(
                              padding: const EdgeInsets.symmetric(
                                horizontal: 14,
                                vertical: 8,
                              ...
                              child: Row(
                                ...
                                children: [
                                  const Icon(
                                    Icons
                                        .groups, // Changed icon to represent groups
                                    color: saffron,
                                    size: 16,
                                  ),
                                  const SizedBox(width: 6),
                                  Flexible(
                                    child: Text(
                                      "GRP: ${adminGroupName?.toUpperCase()}", // Displaying the human-readable group name
